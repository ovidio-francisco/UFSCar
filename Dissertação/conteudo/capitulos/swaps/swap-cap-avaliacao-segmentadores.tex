...
configurações colocadas na primeira posição do \textit{ranking}.

. A Tabela~\ref{tab:configuracoes-ranking-um} contém os


e em seguida, executou-se o novamente o teste com as melhores configurações de cada algoritmo/medida.  Na Tabela~\ref{tab:configuracoes-ranking-um} é apresentada o resultado da primeira etapa, a qual contém as configurações colocadas na primeira posição do \textit{ranking}.




Inicialmente foi gerado um \textit{ranking} para cada algoritmo/medida e selecionou-se a primeira configuração de cada \textit{ranking} os quais são apresentados na Tabela~\ref{tab:configuracoes-ranking-um}.
% --------------------------------------------------------------------------------------



Esse teste é utilizado para...
Pelo diagrama, pode-se observar que os algoritmos propostos baseados em redes compostas
por relações do tipo documento-termo e termo-termo, IMHN C (DT-TT) e IMHN R (DT-
TT), obtiveram as primeiras posições do ranking.


Este é um teste não paramétrico, baseado em ranking e aconselhável para a análise estatística dos resultados de múltiplos classificadores para múltiplas coleções (Trawinski et al., 2012; García et al., 2010; Demsar, 2006).





Na Figuras 3.7 e 3.8 são apresentados os diagramas de diferença crítica resultantes do teste de significância estatística de Friedman

......

Na Figura 3.4 é apresentado o diagrama de diferença crítica que corresponde à uma ilustração gráfica do resultado do teste estatístico.

......

Neste diagrama é exibido o ranking médio dos algoritmos e aqueles conectados por uma linha (diferença crítica) não apresentam diferenças estatisticamente significantes. Novamente, só foram considerados os resultados das coleções nas quais foi possível gerar resultados para todas as configurações apresentadas na seção anterior.


......


Devido à grande quantidade de algoritmos utilizados nesta avaliação experimental, foram criados 5 grupos de experimentos para melhor visualização, análise e compreensão dos resultados. No primeiro grupo
...
% gerou-se um ranking das melhores configurações de cada algoritmo.  
% a fim de encontrar as melhores configurações de cada algoritmo.
...
% \begin{figure}[!ht] \centering     %%% not \center
		...
		% \includegraphics[width=1\textwidth]{conteudo/capitulos/figs/cds.png}	
...
%% Inicialmente cada algoritmo foi executado com 
% Inicialmente foi gerado um \textit{ranking} para cada algoritmo/medida e em seguida, executou-se o novamente o teste com as melhores configurações de cada algoritmo/medida.  Na Tabela~\ref{tab:configuracoes-ranking-um} é apresentada o resultado da primeira etapa, a qual contém as configurações colocadas na primeira posição do \textit{ranking}.
...
50,69,73,79,209,186,71,140,130
