...
O texto a ser segmentado frequentemente é extraído de documentos do tipo \textit{pdf}, \textit{doc}, \textit{docx} ou \textit{odt} que normalmente possuem formato binário. A fim de extrair o texto desses documentos, aplicou-se um processo que transforma esses formatos em aquivos de texto plano. 
Após isso, uma ata tem em média 906 \textit{tokens}, incluindo elementos menos informativos. A fim de preparar o texto e selecionar as palavras mais significativas, este passa por processos de transformação os quais serão apresentados a seguir.
...
% Cabeçalhos e rodapés
\item Remoção de cabeçalhos e rodapés: as atas frequentemente contém trechos que podem ser considerados pouco informativos e descartados durante o pré-processamento, como cabeçalhos e rodapés que se misturam aos tópicos tratados na reunião, podendo ser  inseridos no meio de um tópico prejudicando tanto o algoritmo de segmentação, quanto a leitura do texto pelo usuário.

% Identificação de sentenças
\item Identificação de finais sentenças: cada final de sentença é identificado e marcado com uma \textit{string} especial, esse processo é melhor descrito na Subseção~\ref{subsec:indentificacaosentencas}.

% Limpeza
%\item Limpeza: 
%Todos os caracteres foram convertidos em caixa baixa e eliminou-se a acentuação, sinais de pontuação e todos os \textit{tokens} menores que três caracteres.
%
%% Numerais
%\item Remoção de numerais:
%Nas atas, é comum o uso de numerais para marcação de páginas e linhas, da mesma forma, são pouco informativos e foram descartados.
%
%% Stop Words
%\item Remoção de \textit{Stop Words}: 
%Removeu-se as palavras consideradas menos informativas, para isso, utilizou-se uma lista contendo 438 palavras. 
...
% Após cada passo, reduziu-se a quantidade de \textit{tokens}. Ao final, extraiu-se em média 441 palavras de cada ata, conforme mostrado na Tabela~\ref{tab:preprocessamento}.
