...
import TETAlgorithms.Proximity;
...
public class BisectingKMeans_NonParametric extends TopicExtractor{
    ...
    public BisectingKMeans_NonParametric(int numDocs, int numTerms, int numTopics, int numMaxIteracoes, double minimunDifference, boolean centroidLabeling, int maxK, double percMinVariation){
    ...
    public void buildTopics(Instances data){
        ...
        double previousCohesion = 1000000;
        double currentCohesion = 0;
        
        boolean sair1 = false;
        while(sair1 == false){
            
            System.out.print("Current Number of Topics: " + centroids.size() + " - ");
            ...
            currentCohesion = 0;
            for(int topic=0;topic<centroids.size();topic++){
                for(int doc=0;doc<docsPerTopic.get(topic).size();doc++){
                    int idDoc = docsPerTopic.get(topic).get(doc);
                    double prox = Proximity.calcDistCosseno(adjacencyListDocTerm[idDoc],centroids.get(topic));
                    currentCohesion += prox;
                }
            }
            currentCohesion = currentCohesion / getNumDocs();
            System.out.print("COHESION: " + currentCohesion + " - ");
        ...
        setNumTopics(centroids.size());
        docTopic = new double[getNumDocs()][getNumTopics()];
        termTopic = new double[getNumTerms()][getNumTopics()];
