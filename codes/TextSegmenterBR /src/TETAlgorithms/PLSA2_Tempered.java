 ...
 * @author Gaia
 */
public class PLSA2_Tempered extends TopicExtractorOld{
    
    private int numMaxInteracoes;
    private double minimumDifference;
    
    public PLSA2_Tempered(int numDocs, int numTerms, int numTopics, int numMaxIteracoes, double minimunDifference){
    ...
    public void buildTopics(Instances data){
        ...
        while(sair == false){
            ...
            beta = beta * 0.8;
            M4MShowStatus.setMessage("---Beta:" + beta);
    ...
    private void EStep(double[] probTopic, double[][] probDocTopic, double[][] probTermTopic, double[][][] probTopicDocTerm, Neighbor[] adjListDocTerm, double beta){
        for(int doc=0;doc<getNumDocs();doc++){
            ArrayList<IndexValue> neighbors = adjListDocTerm[doc].getNeighbors();
            for(int term=0;term<neighbors.size();term++){
                double norm=0;
                for(int topic=0;topic<getNumTopics();topic++){
                    double valor = probTopic[topic] * Math.pow((probDocTopic[doc][topic] * probTermTopic[neighbors.get(term).index][topic]), beta);
