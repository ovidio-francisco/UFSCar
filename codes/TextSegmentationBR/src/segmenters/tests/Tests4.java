...
public class Tests4 {
	...
	public static ArrayList<ArrayList<EvaluationData>> doTests(SegmenterAlgorithms algorithm, boolean useMyPreprocess) {
		...
		if (algorithm == SegmenterAlgorithms.TEXT_TILING || algorithm == null) {
			TestSegmenterModel textTilingModel = new TestSegmenterModel(SegmenterAlgorithms.TEXT_TILING);
		...
		if (algorithm == SegmenterAlgorithms.BAYESSEG || algorithm == null) {
			TestSegmenterModel bayesSegModel = new TestSegmenterModel(SegmenterAlgorithms.BAYESSEG);
			bayesSegModel.setPreprocess(preprocess);
			result.add(Tests.testAll(folder, bayesSegModel));
		}

		if (algorithm == SegmenterAlgorithms.MINCUT || algorithm == null) {
			TestSegmenterModel minCutModel = new TestSegmenterModel(SegmenterAlgorithms.MINCUT);
			minCutModel.setPreprocess(preprocess);
			result.add(Tests.testAll(folder, minCutModel));
		}

		if (algorithm == SegmenterAlgorithms.TEXT_SEG || algorithm == null) {
			TestSegmenterModel textSegModel = new TestSegmenterModel(SegmenterAlgorithms.TEXT_SEG);
			textSegModel.setPreprocess(preprocess);
			result.add(Tests.testAll(folder, textSegModel));
		}
...
//			}
	...
	public static ArrayList<String> createTexTableLines(SegmenterAlgorithms algorithm, boolean useMyPreprocess, boolean includeAlgLabel) {
		...
		String format = "%.3f";
		System.out.println("Great TeX Table" + evaluations.size());
		...
		for(ArrayList<EvaluationData> ev : evaluations) {
			if(includeAlgLabel) {
				configs.add(ev.get(0).getAlg().getLabel());//getAlgLabel());
			}
